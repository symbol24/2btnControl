...
func _play(audio:AudioFile = null, _is_2d := false):
	var new_player = TBAudioStreamPlayer.new()
	if audio != null:
...
func _update_audio_volume(bus_name := "Master", percent := 1.0):
